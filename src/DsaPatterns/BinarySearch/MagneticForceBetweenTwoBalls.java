...
public class MagneticForceBetweenTwoBalls {
    ...
    public static int maxDistance(int[] position, int m) {
        ...
        int res=-1;
        while(low<=high){
            int guessedForce=low+(high-low)/2;
            if(allBallsCanBePlacedWithForce(position,m,guessedForce)) {
                res=guessedForce;
                low=guessedForce+1;
            }
            else{
    ...
    public static boolean allBallsCanBePlacedWithForce(int[] position, int balls, int guessedForce){
        int ballsPlaced=1;
        int prevPosition = position[0];
        for(int i=1;i<position.length;i++){
            if(position[i]-prevPosition>=guessedForce){
                ballsPlaced++;
